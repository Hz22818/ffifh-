...
def generate_start_message():
    ...
    markup.row(InlineKeyboardButton("👨‍💻 Мой профиль", callback_data='my_profile'),
               InlineKeyboardButton("🎭 Смена роли", callback_data='change_role'))
    markup.row(InlineKeyboardButton("♻️ Токены", callback_data='tokens'),
               InlineKeyboardButton("❤️ Помощь", callback_data='help'))
    markup.row(InlineKeyboardButton("🎁 Получить бесплатные токены", callback_data='free_tokens'))
    welcome_text = (
        "Привет! Я бот, который поможет тебе использовать нейросеть Chat GPT. "
        "Для начала, тебе нужно получить токены. Используй кнопку 'Токены', чтобы узнать как это сделать. "
...
def generate_change_role_message(selected_role=None):
    roles = [
        "Обычный ассистент 🌐", "Рекламодатель 📣",
        "Мотивационный спикер 🎙", "Переводчик смайликов 😀",
        "CEO Компании 👨‍⚖️", "Математик ➗",
        "Финансовый аналитик 💰", "DAN (Хакнутый GPT) 😈",
        "Анти-плагиат 🚫", "Сценарист 📝",
        "Историк 📚", "Программист 👨‍💻",
        "В двух словах 🗣", "Психолог 🧠"
    ]
    ...
    change_role_text = "*Выбери роль для GPT:*\nПо умолчанию - Обычный ассистент 🌐"
...
async def process_my_profile(callback_query: types.CallbackQuery):
    ...
    profile_text = (
        ...
        "🌐 Обычный ассистент: `0`\n"
        "📣 Рекламодатель: `0`\n"
        "🎙 Мотивационный спикер: `0`\n"
        "😀 Переводчик смайликов: `0`\n"
        "👨‍⚖️ CEO компании: `0`\n"
        "➗ Математик: `0`\n"
        "💰 Финансовый аналитик: `0`\n"
        "😈 DAN (Хакнутый GPT): `0`\n"
        "🚫 Анти-плагиат: `0`\n"
        "📝 Сценарист: `0`\n"
        "📚 Историк: `0`\n"
...
async def process_tokens(callback_query: types.CallbackQuery):
    ...
    tokens_text = (
        ...
        "Мы предоставляем 25 токенов *бесплатно верифицированным пользователям*. Они обновляются каждую неделю. "
        ...
        "*По подписке Plus вам дается 25 токенов *ежедневно* и 15 новых ролей для нейросети.*\n\n"
        "*Стоимость:*\n"
        "4.5$ \\ 30 дней\n"
        "23$ \\ 6 месяцев\n"
    ...
    tokens_markup = InlineKeyboardMarkup().row(
        InlineKeyboardButton("4.5$ (30 дней)", url="t.me/CryptoBot?start=IVmzQnqukQLi"),
        InlineKeyboardButton("23$ (6 месяцев)", url="t.me/CryptoBot?start=IVYASNApd06G")
    ).row(
        InlineKeyboardButton("🎁 Получить бесплатные токены", callback_data='get_free_tokens')
    ).row(
        InlineKeyboardButton("⇠ Назад", callback_data='back_to_main_menu')
    )
...
async def process_help(callback_query: types.CallbackQuery):
    ...
    help_text = (
        "Вы можете задавать боту вопросы из абсолютно любых областей, в том числе *из области философии, "
        "маркетинга, психологии, математики и технических заданий, а также просить написать код* и многое другое. "
        "Вы можете получить ответы на любые вопросы и темы за несколько секунд, что значительно экономит ваше "
        "время и деньги.\n\n"
        "Чтобы получить максимально качественный и корректный ответ от искусственного интеллекта, нужно соблюдать "
        "несколько правил. Важно поставить вопрос максимально точно и корректно, чтобы AI не додумывал ничего лишнего.\n"
        "⛔️ *Пример плохого запроса*: _«Напиши мне лучший способ заработка?»_ - в этом запросе нет конкретики, "
        "поэтому ответ будет слабым.\n\n"
        "✅ *Пример хорошего запроса*: _«Напиши мне 3 лучших стратегии заработка денег, учитывая, что мне 21 год, "
        "я могу работать по 45 часов в неделю и я хочу работать на себя, а не на компанию. Я владею навыками "
        "программирования и хорошо знаю английский.»_ - в этом запросе мы указали конкретные данные, которые "
        "помогут AI дать нам грамотный ответ, учитывая наши потребности.\n\n"
        "Чтобы получить более корректные и ёмкие ответы, нужно разбить тему на несколько подтем. Таким образом, "
        "вы получите несколько ответов, которые будут более качественными и проработанными.\n\n"
        "Бот помнит ваши предыдущие вопросы и темы разговоров, что значительно упрощает работу и взаимодействие с ним. "
        "Вы можете попросить написать рекламный пост для вашей социальной сети, а после ответа от бота попросить "
        "добавить креативности или кликбейта. ChatGPT поймет, о чем речь, и добавит в сгенерированный пост мотивационного "
        "контекста и кликбейтовый заголовок.\n\n"
        "*Если у вас остались какие-либо вопросы, вы всегда можете воспользоваться нашей службой поддержки - @support_bot*"
    )
